    ...
    class GamesController < ApplicationController
      ...
      def question
        if @game.sudden_death?
          # For sudden death, just check if we have a question
          q = current_question
          ...
          ok({
            ...
            text: q.text,
            options: q.options,
            ends_at: @game.question_end_at&.iso8601(3) || (q.time_limit.seconds.from_now.iso8601(3))
          })
        else
          # For regular rounds, check timer
      ...
      def round_result
        ...
        ActiveRecord::Base.transaction do
          if lowest.size == 1
            ...
            # Set up sudden death with proper round context
            @game.update!(
              status: :sudden_death, 
              sudden_death_candidate_ids: lowest.map { |s| s[:player].id },
              round_number: 4,  # Use round 4 for sudden death questions
              current_question_index: 0  # Start with first sudden death question
            )
            # Store the original round where the tie occurred in the sudden_death_candidate_ids
            # We'll use a special format: [original_round, player_id1, player_id2, ...]
            original_round = round
            candidate_ids = lowest.map { |s| s[:player].id }
            @game.update!(sudden_death_candidate_ids: [original_round] + candidate_ids)
            next_state = :sudden_death
          end
      ...
      def sudden_death_resolve
        ...
        candidate_data = @game.sudden_death_candidate_ids
        original_round = candidate_data.first  # First element is the original round
        candidate_ids = candidate_data[1..-1]  # Rest are player IDs
        subs = Submission.where(game: @game, question: q, player_id: candidate_ids)
        correct_subs = subs.where(correct: true)
        ...
        if correct_subs.any?
          ...
          # Go back to the original round and mark it as completed, then advance to next round
          next_round = original_round + 1
          @game.update!(
            status: :between_rounds, 
            sudden_death_candidate_ids: [],
          ...
          # No correct answers - eliminate the slowest player and end sudden death
          slowest_sub = subs.order(latency_ms: :desc).first
          if slowest_sub
            eliminated_player = slowest_sub.player
            eliminated_player.update!(eliminated: true)
            eliminated_names = [eliminated_player.name]
          else
            # If no submissions, eliminate a random player
            eliminated_player = Player.where(game: @game, id: candidate_ids).first
            eliminated_player.update!(eliminated: true)
            eliminated_names = [eliminated_player.name]
          end

          # Reset to original round and end sudden death
          # Go back to the round where the tie occurred and mark it as completed, then advance to next round
          next_round = original_round + 1
          @game.update!(
            status: :between_rounds, 
            sudden_death_candidate_ids: [], 
          ...
          broadcast(:sudden_death_result, {
            winner: { id: eliminated_player.id, name: eliminated_player.name },
            eliminated_names: eliminated_names,
            next_state: :between_rounds,
            leaderboard: leaderboard
          })

          ok({ winner: eliminated_player.name, eliminated: eliminated_names, next_state: :between_rounds, leaderboard: leaderboard })
        end
      end
