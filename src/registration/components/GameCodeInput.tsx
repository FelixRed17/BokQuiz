...
export const GameCodeInput: React.FC<GameCodeInputProps> = ({ value, onChange, error, disabled, onEnterPress }) => {
  ...
  const getInputClasses = () => {
    let classes = 'w-full h-12 px-4 rounded-lg border-2 transition-all duration-300 text-center uppercase tracking-wider'
    classes += ' text-gray-800 text-base font-medium bg-gray-50'
    classes += ' box-border'

    if (error) {
      classes += ' border-red-500 bg-red-50'
    } else if (value.trim() && ValidationService.validateGameCode(value).isValid) {
      ...
      >
        <input
          ...
          onFocus={(e) => {
            ...
            e.target.style.boxShadow = '0 0 0 3px rgba(255, 215, 0, 0.1), 0 4px 12px rgba(0, 0, 0, 0.1)'
